...
class NLXmodel(nn.Module):
    ...
    def __init__(self):
        ...
        config_up = AutoConfig.from_pretrained('ckpts/nle_upper_model_4')
        config_down = AutoConfig.from_pretrained('ckpts/nle_lower_model_4')
        # Add configs
        ...
        self.upper_model = GPT2LMHeadModel.from_pretrained('ckpts/nle_upper_model_4', config=config_up)
        self.lower_model = GPT2LMHeadModel.from_pretrained('ckpts/nle_lower_model_4', config=config_down)
    ...
    def generate_greedy(self, tokenizer, prompt_ids, prompt_seg_ids, img_embeddings, up_hidden_states, max_len = 20):
        ...
        current_output_exp = []
        input_ques_exp_ids = prompt_ids
        ques_exp_segment_ids = prompt_seg_ids
    ...
    def forward(self, img_embeddings, input_ques_ans_ids, input_ques_exp_ids, labels4ques_ans, labels4ques_exp,
       ...
       # if reinforcement_flag == True:
        ...
        for batch_index, label in zip(range(answer_logits.shape[0]), prediction_list):
            ...
            greedy_img_embeddings = img_embeddings[batch_index].unsqueeze(0)
            greedy_up_hidden_states = weight_for_answer_tensor[batch_index].unsqueeze(0)
            ...
            if 780 in input_ques_exp_ids[batch_index]:
                ...
                identifier = True
                greedy_texts = []
                greedy_texts.append(gt_explantion)
                greedy_texts[0] = ' ' + greedy_texts[0]
            else:
                identifier = False

                greedy_texts = self.generate_greedy(tokenizer, tokenized_prompt_ids, question_seg_ids,
                                                    greedy_img_embeddings, greedy_up_hidden_states)
                greedy_texts = tokenizer.decode(greedy_texts)
                greedy_texts = [greedy_texts]


            explanation_ids = torch.tensor(tokenizer(greedy_texts[0])['input_ids'],
            ...
            reward_logits = self.upper_model(
                ...
                encoder_hidden_states=greedy_img_embeddings,
                encoder_attention_mask=None,
                labels=None,
                use_cache=False,
                return_dict=True,
            ...
            if identifier == False:
                ...
                gt_score = gt_score / base_logits_softmax.shape[1]
                reward_score = reward_score / reward_logits_softmax.shape[1]

                loss_r = torch.clamp((reward_score - gt_score) * 10, min=0.) + loss_r
                batch_norm_r += 1
    ...
    for i, batch in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            for batch_index in range(answer_logits.shape[0]):
                ...
                greedy_img_embeddings = img_embeddings[batch_index].unsqueeze(0)
                greedy_up_hidden_states = weight_for_answer[batch_index].unsqueeze(0)

                greedy_texts = model.generate_greedy(tokenizer, tokenized_prompt_ids, question_seg_ids,
                                                         greedy_img_embeddings, greedy_up_hidden_states)
        ...
        current_output_exp = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(exp_greedy_texts[0]))
